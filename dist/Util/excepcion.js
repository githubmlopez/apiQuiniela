...
export async function createExcepcion(data, logger, sequelize) {
    try {
        //const modelo = await def_FC_TAREA_EVENTO(sequelize);
        const nomModelo = 'FC_TAREA_EVENTO';
        const modelo = sequelize.models[nomModelo];
...
export function crearObjError(error, contexto) {
    let msgErrorUs = 'Ha ocurrido un error inesperado.';
    let msgErrorSis = `Error en el contexto '${contexto}': No se pudo determinar el tipo de error.`;
    let errorStack = undefined; // Changed to undefined for consistency
    ...
    // --- Manejo de Errores Estándar de JavaScript ---
        ...
        if (error.name === 'AggregateError' && error.errors && Array.isArray(error.errors)) {
            const aggregatedMessages = error.errors.map((e) => e.message || 'Error interno sin mensaje').join('; ');
...
export async function crearObjetoEvento(error, opciones, contexto) {
    console.log('✅ Objeto error II', error);
    const infError = crearObjError(error, contexto);
    const data = {
